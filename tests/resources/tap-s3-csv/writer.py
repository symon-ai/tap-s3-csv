...
# Use a cryptographically secure RNG (CSPRNG) sourced from os.urandom via
# random.SystemRandom to satisfy CWE-331 (Insufficient Entropy). It exposes the
# same method surface as the module-level random API (random/randint/uniform),
# so the generated test-data semantics (ranges, counts, formatting) are
# preserved while no bare non-CSPRNG module-level calls remain.
