...
# CWE-331: use a cryptographically secure RNG (backed by os.urandom) instead of
# the default non-cryptographic Mersenne Twister PRNG. SystemRandom exposes the
# same .random()/.randint()/.uniform() API, so this is a drop-in replacement that
# preserves the generated-fixture shape and ranges.
