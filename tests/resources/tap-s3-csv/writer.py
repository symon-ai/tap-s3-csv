...
# Use a cryptographically-secure PRNG (backed by os.urandom) instead of the
# module-level `random` functions to satisfy CWE-331 (Insufficient Entropy).
...
def test_bigint_valid_range():
    ...
    return [["bigint_signed"] +
            [_SYS_RANDOM.randint(-2**63, 2**63-1) for _ in range(95)] +
            [-2**63, 2**63-1, -1, 0, 1]]
...
def test_unsigned_bigint_valid_range():
    ...
    return [["bigint_unsigned"] +
            [_SYS_RANDOM.randint(2**63, 2**64) for _ in range(98)] +
            [-2**63, 2**63-1, -1, 0, 1]]
...
def test_beyond_bigint_valid_range():
    ...
    return [["larger_than_bigint"] +
            [_SYS_RANDOM.randint(2**64+1, 2**65) for _ in range(49)] +
            [_SYS_RANDOM.randint(-2**64, -2**63-1) for _ in range(49)] +
            [2**64+1, -2**63-1]]
...
def test_float_double_representable_range():
    ...
    data = [["float at the edges", "double at the edges"]] + \
           [[_SYS_RANDOM.uniform(MIN_POS_FLOAT, MAX_POS_FLOAT),
             _SYS_RANDOM.uniform(MIN_POS_DOUBLE, MAX_POS_DOUBLE)] for _ in range(47)] + \
           [[_SYS_RANDOM.uniform(MIN_NEG_FLOAT, MIN_NEG_FLOAT),
             _SYS_RANDOM.uniform(MIN_NEG_DOUBLE, MAX_NEG_DOUBLE)] for _ in range(47)] + \
...
def test_float_double_normal_range():
    return [["float with precision"] +
            [_SYS_RANDOM.random() for _ in range(25)] +
            [_SYS_RANDOM.random() * 10 ** 16 for _ in range(25)] +
            [-_SYS_RANDOM.random() for _ in range(25)] +
            [_SYS_RANDOM.random() * 10 ** 16 for _ in range(25)]]
...
def test_number_formats_commas():
    ...
    data = [["positive numbers with commas", "negative numbers with commas"]] + \
           [["{:,}".format(_SYS_RANDOM.random() * 10 ** 10),
             "{:,}".format(-_SYS_RANDOM.random() * 10 ** 10)] for _ in range(100)]
...
def test_number_formats_plus_minus_signs():
    ...
    data = [["positive with various signs", "negative with signs"]] + \
           [["{:+f}".format(_SYS_RANDOM.random() * 10 ** 10),
             "{:+f}".format(-_SYS_RANDOM.random() * 10 ** 10)] for _ in range(34)] + \
           [["{: f}".format(_SYS_RANDOM.random() * 10 ** 10),
             "{: f}".format(-_SYS_RANDOM.random() * 10 ** 10)] for _ in range(34)] + \
           [["{:-f}".format(_SYS_RANDOM.random() * 10 ** 10),
             "{:-f}".format(-_SYS_RANDOM.random() * 10 ** 10)] for _ in range(32)]
...
def test_number_formats_percentages():
    ...
    data = [["positive percentages", "negative percentages"]] + \
           [["{:%}".format(_SYS_RANDOM.random()), "{:%}".format(-_SYS_RANDOM.random())] for _ in range(100)]
...
def test_date_time_iso_format():
    ...
    data = []
    for row in range(100):
        timezone = datetime.timezone(datetime.timedelta(minutes=_SYS_RANDOM.randint(0, 59), hours=_SYS_RANDOM.randint(-23, 23)),
                                     "ABC")
        dt = datetime.datetime.now(tz=timezone) + datetime.timedelta(
            seconds=_SYS_RANDOM.uniform(-60 * 60 * 24 * 365 * 100, 60 * 60 * 24 * 365 * 100))
...
def test_date_time_iso_format_to_hours():
    ...
    for row in range(100):
        timezone = datetime.timezone(
            datetime.timedelta(minutes=_SYS_RANDOM.randint(0, 59), hours=_SYS_RANDOM.randint(-23, 23)),
            "ABC")
        dt = datetime.datetime.now(tz=timezone) + datetime.timedelta(
            seconds=_SYS_RANDOM.uniform(-60 * 60 * 24 * 365 * 100, 60 * 60 * 24 * 365 * 100))
...
def test_primary_key_unique_values_and_nullable_integers():
    ...
    data = [["key", "nullable integer", "non-nullable integer", "description"]] + \
           [[key,
             _SYS_RANDOM.randint(1, 1000) if key != 11 else None,
             _SYS_RANDOM.randint(1, 1000) if key != 12 else None,
...
def test_primary_key_56_null_config(quoting: csv = csv.QUOTE_MINIMAL):
    with open('test_primary_key_56_null.csv', 'w') as file:
        ...
        for pk in range(1, 100):
            if pk != 56:
                write.writerow([pk, _SYS_RANDOM.randint(1, 1000)])
            else:
                write.writerow([None, _SYS_RANDOM.randint(1, 1000)])


# The JSON for this file contains regex pattern matching that matches MULTIPLE files
def test_multiple_file_with_pk_part_a():
    data = [["key", "value"]] + \
           [[key, _SYS_RANDOM.randint(1, 1000)] for key in range(1, 20)]
...
def test_multiple_file_with_pk_part_b():
    data = [["key", "value"]] + \
           [[key, _SYS_RANDOM.randint(1, 1000)] for key in range(21, 40)]
...
def test_multiple_file_with_pk_part_c():
    data = [["key", "value"]] + \
           [[key, _SYS_RANDOM.randint(1, 1000)] for key in range(61, 80)]
...
def test_header_order_multiple_file_with_pk_part_a():
    data = [["key", "value"]] + \
           [[key, _SYS_RANDOM.randint(1, 1000)] for key in range(41, 60)]
...
def test_header_order_multiple_file_with_pk_part_b():
    data = [["value", "key"]] + \
           [[_SYS_RANDOM.randint(1, 1000), key] for key in range(61, 80)]
...
def test_primary_key_56_duplicate_config(quoting: csv = csv.QUOTE_MINIMAL):
    with open('test_primary_key_56_duplicate.csv', 'w') as file:
        ...
        for pk in range(1, 100):
            write.writerow([pk, _SYS_RANDOM.randint(1, 1000)])
        write.writerow([56, _SYS_RANDOM.randint(1, 1000)])
