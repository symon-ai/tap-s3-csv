...
# Use a cryptographically-secure random source (CWE-331 remediation).
# secrets.SystemRandom is a random.Random subclass backed by os.urandom and
# exposes the same API (.uniform / .random / .randint), so the fixture's
# numeric behaviour and CSV output shape are preserved.
